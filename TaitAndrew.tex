...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Custom Corpus Function for preprocessing  }

buildCorpus <- \hlkwd{function} (tweets, wholeDataSet, wordAssocation)\{
  corp <- \hlkwd{Corpus}(\hlkwd{VectorSource}(tweets$text))
  
  
  corp <- \hlkwd{tm_map}(corp,
                 \hlkwd{content_transformer}(\hlkwd{function}(x) \hlkwd{iconv}(x, to=\hlstr{'ASCII'},
                                                       sub=\hlstr{'byte'})))
\hlcom{  # remove stop words and other preprocessing}
 
  corp <- \hlkwd{tm_map}(corp, \hlkwd{content_transformer}(tolower))
  corp <- \hlkwd{tm_map}(corp, removeNumbers)

  toSpace = \hlkwd{content_transformer}( \hlkwd{function}(x, pattern) \hlkwd{gsub}(pattern,\hlstr{" "},x) )
  
  
\hlcom{  ##Tweet cleaning}
  
\hlcom{  #credit to https://stackoverflow.com/a/31352005/8816204}
  corp <-\hlkwd{tm_map}(corp, toSpace, \hlstr{"(RT|via)((?:\textbackslash{}\textbackslash{}b\textbackslash{}\textbackslash{}W*@\textbackslash{}\textbackslash{}w+)+)"})
  corp <-\hlkwd{tm_map}(corp, toSpace, \hlstr{"@\textbackslash{}\textbackslash{}w+"})
  corp <-\hlkwd{tm_map}(corp, toSpace, \hlstr{"&amp"})
  
\hlcom{  ##Remove URLS from tweets}
  corp <-\hlkwd{tm_map}(corp, toSpace, \hlstr{"httpsw+"})
  corp <-\hlkwd{tm_map}(corp, toSpace, \hlstr{"http:\textbackslash{}\textbackslash{}w+"})
  corp <-\hlkwd{tm_map}(corp, toSpace, \hlstr{"https:\textbackslash{}\textbackslash{}w+"})

  corp <-\hlkwd{tm_map}(corp, toSpace, \hlstr{"[ \textbackslash{}t]\{2,\}"})
  corp <-\hlkwd{tm_map}(corp, toSpace, \hlstr{"^\textbackslash{}\textbackslash{}s+|\textbackslash{}\textbackslash{}s+$"})
  

\hlcom{  # ##Remove obvious words /stopwords}
  \hlkwd{if}(\hlkwd{missing}(wholeDataSet))\{
    corp <- \hlkwd{tm_map}(corp, 
                   \hlkwd{function}(x)\hlkwd{removeWords}(x,\hlkwd{c}(
                     \hlkwd{stopwords}(\hlstr{"english"}),\hlstr{"amp"}, \hlstr{"will"}, \hlstr{"\textbackslash{}‰Û_"}, \hlstr{"https"}, \hlstr{"http"}, \hlstr{"httpsdb"})))
  \} else \hlkwd{if}(wholeDataSet == TRUE)\{
    corp <- \hlkwd{tm_map}(corp, 
                   \hlkwd{function}(x)\hlkwd{removeWords}(x,\hlkwd{c}(
                     \hlkwd{stopwords}(\hlstr{"english"}),\hlstr{"amp"}, \hlstr{"will"}, \hlstr{"\textbackslash{}‰Û_"}, \hlstr{"https"}, \hlstr{"http"}, 
                     \hlstr{"httpsdb"}, \hlstr{"eu"}, \hlstr{"brexit"}, \hlstr{"rt"}, \hlstr{"leave"}, \hlstr{"remain"}, \hlstr{"vote"})))
  \}

\hlcom{  #remove punctuation last so urls are removed correctly}
  corp <- \hlkwd{tm_map}(corp, removePunctuation)
  
  \hlkwd{if}(\hlkwd{missing}(wordAssocation))\{
    tdm <- \hlkwd{TermDocumentMatrix}(corp)
    
    m <- \hlkwd{as.matrix}(tdm)
    v <- \hlkwd{sort}(\hlkwd{rowSums}(m), decreasing = TRUE)
    d <- \hlkwd{data.frame}(word = \hlkwd{names}(v), freq = v)
    d$word <- \hlkwd{gsub}(\hlstr{"˜"}, \hlstr{" "}, d$word) ## Edit 2
    
    tweets <- d$word
  \}else \hlkwd{if}(wordAssocation == TRUE)\{
    dtm <- \hlkwd{DocumentTermMatrix}(corp)
  \}
\}
\end{alltt}
\end{kframe}
